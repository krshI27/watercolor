...
class Paper:
    ...
    def __init__(self, width: int, height: int, c_min: float = 0.3, c_max: float = 0.7):
        ...
        # Initialize fields
        self.height_field = np.random.randn(
            height, width
        )  # Add randomness for initial state
    ...
    def _generate_fractal(self):
        ...
        for i in range(octaves):
            ...
            noise_height = max(1, self.height // freq)
            noise_width = max(1, self.width // freq)
            noise = np.random.randn(noise_height, noise_width)
    ...
    def update_capacity(self):
        ...
        self.fluid_capacity = self.height_field * (self.c_max - self.c_min) + self.c_min
