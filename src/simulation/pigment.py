...
class Pigment:
    ...
    def set_km_params_from_colors(
        self, white_color: np.ndarray, black_color: np.ndarray
    ) -> None:
        """
        ...
        white_color = np.asarray(white_color)
        black_color = np.asarray(black_color)

        # Ensure values are in valid range [0, 1]
        white_color = np.clip(white_color, 0.001, 0.999)  # Avoid exact 0 or 1
        black_color = np.clip(
            black_color, 0.001, white_color
        ...
        # Replace any invalid values (NaN or negative) with safe defaults
        K = np.nan_to_num(K, nan=0.0, posinf=1e6, neginf=0.0)
        S = np.nan_to_num(S, nan=1e-6, posinf=1e6, neginf=1e-6)  # S shouldn't be 0

        # Ensure K and S are non-negative
        K = np.maximum(K, 0.0)
        S = np.maximum(S, 1e-6)
...
class PigmentLayer:
    ...
    def set_water_concentration(
        self, mask: np.ndarray, concentration: float = 1.0
    ) -> None:
        """
    ...
    def transfer_pigment(self, paper_height: np.ndarray, wet_mask: np.ndarray) -> None:
        ...
        if len(wet_indices[0]) > 0:
            ...
            delta_down = (
                g[wet_indices]
                * (1.0 - paper_height[wet_indices] * granularity)
                * density
            )
            ...
            delta_up = (
                d[wet_indices]
                * (1.0 + (paper_height[wet_indices] - 1.0) * granularity)
                * density
                / staining_power
            )
