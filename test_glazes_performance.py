...
from auto_watercolorize import create_paper_structure, create_wetness_distribution, run_simulation_chunk
...
def test_simulation_stages():
    ...
    for _ in range(steps):
        ...
        sim.simulate_capillary_flow()
    end_time = time.time()
    print(f"{steps} steps: {end_time - start_time:.4f} seconds ({(end_time - start_time)/steps:.4f} seconds per step)")

def optimize_glazes_function():
    """Create an optimized version of the create_glazes function."""
    from auto_watercolorize import create_glazes
